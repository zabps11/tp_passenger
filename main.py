...
myListBus = [] #liste de tous mes bus creer
myVacanPassgerList = []  # liste de tous les passagers enregistreés et non assigner a un bus
response="m"
...
def createBus(): #creation d'un bus
    ...
    while response != 'n':
        ...
        currentBus["id"] = input("==> Entrer l'ID du bus (Imatriculation) :  ").upper()
        for bus in myListBus:
            while bus["id"]==currentBus["id"]:
                print("    un autre bus a deja le meme ID")
                currentBus["id"]=input("    Entrer l'ID du bus (Imatriculation) :  ").lower()
        currentBus["availablePlace"] = int(input("    Entrer le nombre de siege (En chiffre e.g 50) :  "))
...
def createPassenger(): #creation d'un passager
    ...
    while response != 'n':
        ...
        for char in currentPassenger["name"]:
            id += ord(char)
        currentPassenger["id"] =str(id) + str(len(myVacanPassgerList))
...
def allBusInfo(): #toutes les information sur tous les bus qui ont deja ete creer
    print("___________Information sur tous les bus ____________")
    print()
    count = 0
    for bus in myListBus:
        print("==> ID imatriculation :  {}  Possible places :  {}  Available places :  {}  Number of Kg in bus :  {}".format(bus["id"],bus["availablePlace"],bus["availablePlace"] - len(bus["listPassenger"]),bus["nbreKg"]))
...
def BusInfo(busId): # toutes les information sur un bus particulier
    count = 0
    for bus in myListBus:
        if busId.upper() == bus["id"]:
            print("==> ID imatriculation :  {}  Possible places :  {}  Available places :  {}  Number of Kg in bus :  {}".format(bus["id"],bus["availablePlace"],bus["availablePlace"] - len(bus["listPassenger"]),bus["nbreKg"]))
            for pssger in bus["listPassenger"]:
                ...
                count += 1
    if count == 0:
        print("==> Aucun passager n'est encore enregistrer dans ce bus ")

def verifiedPassgerInBus(passengerName): #verifier si un passager est dans un bus
    status=0
    thebus={}
    for bus in myListBus:
        for pssger in bus["listPassenger"]:
...
def verifiedTransfer(BusCommingId,BusGoingId): # verifier la possibilite de transfert de passager entre deux bus different
    ...
    status=0
    for bus in myListBus:
        if BusCommingId.upper() == bus["id"]:
            commingBus = bus
            status += 1
        elif BusGoingId.upper() == bus["id"] :
            goingBus = bus
            status += 1
    if status == 1 or status == 0 :
        ...
        if placeUseInBuscomming <= placeAvailableInBusGoing :
            print("==> le transfert de passagers du bus d'imatriculation {} vers le bus d'imatriculation {} est possible".format(commingBus["id"],goingBus["id"]))
        else:
            print("==> Le transfert de passager du bus {} vers le bus {} est impossible".format(BusCommingId,BusGoingId))

def myswitcher(choix): #mon switcher de menu
    ...
    if choix == "a":
        ...
        #     print("==> ID :  {}  Possible places :  {}  Available places :  {}  number of Kg in bus :  {}".format(bus["id"],bus["availablePlace"],bus["availablePlace"] - len(bus["listPassenger"]),bus["nbreKg"]))
        # print()
        id = input("==> Entrer l'id du bus (Imatriculation) :  ")
        printBuspassgerList(id)
    elif choix == "g":
